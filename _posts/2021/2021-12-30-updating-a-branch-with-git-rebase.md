...
In this pro tip I will show you how to use `git rebase` to update a branch, bringing in the latest commits from main (or master) so your working branch is current.
...
## Updating your branch from main with git rebase
...
Wrapping up, we have an important thing to point out now that our branch is updated: the rebase will only happen without interruptions, as shown in this pro tip blog post, if there are no conflicts, otherwise the rebase will be suspended and the [conflicts must be resolved](https://jtemporal.com/solving-conflicts/) before continuing.

Now you know how to update a branch using rebase, if you want more details about the command `git rebase`, I recommend [reading its documentation](https://git-scm.com/docs/git-rebase/). And if a rebase ever goes sideways and you lose a commit, the [git reflog](https://jtemporal.com/recovering-lost-commits-with-git-reflog/) can help you get it back.
