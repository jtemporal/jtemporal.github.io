...
For hidden `type: video` transcript posts (Medium syndication), follow **`AGENTS.md`** in the repo root. Key rules: one git worktree per post, preview locally before committing, do not push or open a PR until the author approves.
